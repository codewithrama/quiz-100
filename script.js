...
const quizData = [
  ...
  },
  {
    ...
    const totalQuestionsSummaryEl = document.getElementById('total-questions-summary');
    const correctAnswersSummaryEl = document.getElementById('correct-answers-summary');
    const incorrectAnswersSummaryEl = document.getElementById('incorrect-answers-summary');
    ...
    const userAnswers = []; // To store user's answers for review
    ...
    function startTimer() {
        timeLeft = 30;
        timerEl.textContent = `Time: 00:${timeLeft < 10 ? '0' : ''}${timeLeft}`;
        timer = setInterval(() => {
            timeLeft--;
            timerEl.textContent = `Time: 00:${timeLeft < 10 ? '0' : ''}${timeLeft}`;
            if (timeLeft <= 0) {
    ...
    function selectOption(selectedButton, selectedAnswer, correctAnswer, explanation) {
        ...
        if (isCorrect) {
            ...
            feedbackEl.textContent = `Incorrect. The correct answer is: ${correctAnswer}. ` + (explanation || "");
    ...
    function handleTimeOut() {
        disableOptions();
        nextBtn.disabled = false;
        feedbackEl.textContent = `Time's up! The correct answer was: ${quizData[currentQuestionIndex].answer}. ` + (quizData[currentQuestionIndex].explanation || "");
        feedbackEl.classList.add('incorrect');
    ...
    function disableOptions() {
        Array.from(optionsContainerEl.children).forEach(button => {
            ...
            // Keep visual feedback for selected/correct/incorrect
            if (!button.classList.contains('correct') && !button.classList.contains('incorrect')) {
                 // button.style.opacity = 0.7; // Dim unselected, non-correct answers
            }
        });
    }
    ...
    function showResults() {
        ...
        totalQuestionsSummaryEl.textContent = quizData.length;
        correctAnswersSummaryEl.textContent = score;
        incorrectAnswersSummaryEl.textContent = quizData.length - score;
    }
        ...
        userAnswers.forEach((ans, index) => {
            ...
            ans.options.forEach(opt => {
                ...
                if (opt === ans.correctAnswer) {
                    optClass = 'correct-answer-text';
                }
                if (opt === ans.selectedAnswer && !ans.isCorrect) {
                    optClass = 'user-answer incorrect';
                } else if (opt === ans.selectedAnswer && ans.isCorrect) {
                     optClass = 'user-answer correct';
                }

                optionsHTML += `<span class="${optClass}">${opt}</span>`;
            });

            item.innerHTML = `
                <p class="review-question">Q${index + 1}: ${ans.question}</p>
                <div class="review-options">
                    ${optionsHTML}
                </div>
                <p>Your answer: <span class="user-answer ${ans.isCorrect ? 'correct' : 'incorrect'}">${ans.selectedAnswer}</span></p>
                ${!ans.isCorrect ? `<p>Correct answer: <span class="correct-answer-text">${ans.correctAnswer}</span></p>` : ''}
            `;
            reviewContentEl.appendChild(item);
        });
    }
